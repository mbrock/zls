...
pub fn build(b: *Build) !void {
    ...
    { // zig build
        const exe = b.addExecutable(.{ .name = "zls", .root_module = exe_module, .use_llvm = use_llvm, .use_lld = use_llvm });
        b.installArtifact(exe);
    }

    { // zig build hover
        const hover_tool = b.addExecutable(.{ .name = "hover", .root_module = b.createModule(.{
...
fn createZLSModule(
    ...
    const lsp_module = b.dependency("lsp_kit_src", .{
        .target = options.target,
        .optimize = options.optimize,
    }).module("lsp");
