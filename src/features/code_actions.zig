...
pub const Builder = struct {
    ...
    pub fn generateCodeActionsInRange(
        ...
        // 4) Existing string literal refactors (only when cursor is in a string)
    ...
    fn generateHoistConstToFieldAction(builder: *Builder, source_index: usize) !void {
        ...
        const func_ty = try builder.analyser.resolveTypeOfNode(.of(func_node, builder.handle));

        // Determine receiver param name by type identity to container
        var buf: [1]Ast.Node.Index = undefined;
        const proto = tree.fullFnProto(&buf, fn_proto).?;
        const container_ty = try builder.analyser.innermostContainer(builder.handle, tree.tokenStart(proto.ast.fn_token));
        ...
        const has_self_param = try Analyser.hasSelfParam(builder.analyser, func_ty.?);
        ...
        const field_type_text = blk_ft: {
            ...
            if (try builder.analyser.resolveTypeOfNode(.of(init_node, builder.handle))) |ty|
                break :blk_ft try ty.stringifyTypeOf(builder.analyser, .{ .truncate_container_decls = false });
            break :blk_ft "var"; // fallback unlikely
        };
        // Compute insertion point: after lbrace
        const lbrace_tok = tree.firstToken(container_node);
        const lbrace_pos = tree.tokenStart(lbrace_tok);
        ...
        const field_indent = blk_ind: {
            if (cdecl.ast.members.len > 0) {
                ...
                break :blk_ind try std.mem.concat(builder.arena, u8, &.{ base, "    " });
            }
        };
        var field_line: std.ArrayList(u8) = .empty;
        // ensure there's a newline before rbrace, rely on formatter for commas
        try field_line.appendSlice(builder.arena, "\n");
        try field_line.appendSlice(builder.arena, field_indent);
        try field_line.appendSlice(builder.arena, var_name);
        try field_line.appendSlice(builder.arena, ": ");
        try field_line.appendSlice(builder.arena, field_type_text);
        try field_line.appendSlice(builder.arena, ",");
        ...
        try assign_line.appendSlice(builder.arena, indent_slice);
        try assign_line.appendSlice(builder.arena, receiver);
        try assign_line.appendSlice(builder.arena, ".");
        try assign_line.appendSlice(builder.arena, var_name);
        try assign_line.appendSlice(builder.arena, " = ");
        try assign_line.appendSlice(builder.arena, tree.source[init_loc.start..init_loc.end]);
        try assign_line.appendSlice(builder.arena, ";\n");
        // Collect identifier references to this var after the decl
        var repls: std.ArrayList(types.TextEdit) = .empty;
        // Replace the decl
        try repls.append(builder.arena, builder.createTextEditLoc(.{ .start = decl_loc.start, .end = decl_end }, assign_line.items));
        // Insert the field before rbrace
        try repls.append(builder.arena, builder.createTextEditPos(lbrace_pos, field_line.items));
        ...
        const decl_handle = (try builder.analyser.lookupSymbolGlobal(builder.handle, var_name, decl_loc.start)) orelse null;
        if (decl_handle) |target_decl| {
            while (tok_i <= end_tok) : (tok_i += 1) {
                ...
                if (try builder.analyser.lookupSymbolGlobal(builder.handle, name, tok_start)) |ref_decl| {
                    if (!ref_decl.eql(target_decl)) continue;
                    const tok_loc = offsets.tokenToLoc(tree, tok_i);
                    var repl_text: std.ArrayList(u8) = .empty;
                    try repl_text.appendSlice(builder.arena, receiver);
                    try repl_text.appendSlice(builder.arena, ".");
                    try repl_text.appendSlice(builder.arena, var_name);
                    try repls.append(builder.arena, builder.createTextEditLoc(tok_loc, repl_text.items));
                }
            }
        }
        // Emit the action
        try builder.actions.append(builder.arena, .{
            .title = try std.fmt.allocPrint(builder.arena, "hoist local to field", .{var_name}),
            .kind = .refactor,
            .isPreferred = false,
            .edit = try builder.createWorkspaceEdit(repls.items),
        });
    }
